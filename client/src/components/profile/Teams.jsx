...
export default function Teams() {
  ...
  return (
    <section>
      ...
      {
        loading ?
          <span className="flex items-center gap-2 text-xl mt-6 mb-2"><AiOutlineLoading size={20} className="animate-spin" /> Cargando</span>
          :
          teams?.length > 0 ?
            <div className="flex flex-col gap-4">
            {
            teams.map((team, index) => (
              <button type="button" key={team?._id} className={"w-full flex items-center p-4 gap-5 rounded-xl relative overflow-hidden transition duration-200 hover:brightness-110" + (!index % 2 ? " bg-gradient-to-t bg-green-600/90 from-green-800/30" : " bg-gradient-to-t bg-green-500/90 from-green-600/30")}>
